// Used by the admin "Resend License Key Email" button on the order detail page.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    await sendLicenseKeyEmail(admin, order, items ?? [])

    return jsonResponse({ ok: true })
  } catch (e) {
    console.error(e)
    return jsonResponse({ error: 'Unexpected error resending email' }, 500)
  }
})
